...
public abstract class Opener {
	...
	public void startOpening(final Player player, final Crate crate, final Location blockLocation) {
		Crates.getOpenHandler().getCratesPlugin().getCrateHandler().addOpening(player.getUniqueId(), this);
	...
	public Winning getWinning(Crate crate) {
		...
		if (crate.getTotalPercentage() > 0) {
			...
			winning = crate.getWinnings().get(Crates.getOpenHandler().getCratesPlugin().getCrateHandler().randInt(0, crate.getWinnings().size() - 1));
		}
		return winning;
	}

	public File getOpenerConfigFile() {
		File openersDir = new File(JavaPlugin.getPlugin(Crates.class).getDataFolder(), "openers");
	...
	protected void finish(final Player player) {
		Bukkit.getScheduler().runTask(getPlugin(), new Runnable() {
			@Override
			public void run() {
				Crates.getOpenHandler().getCratesPlugin().getCrateHandler().removeOpening(player.getUniqueId());
			}
		});
	}
