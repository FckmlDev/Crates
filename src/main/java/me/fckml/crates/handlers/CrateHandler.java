...
public class CrateHandler {
    ...
    public CrateHandler(Crates crates) {
        this.crates = crates;

        YamlConfiguration dataConfig = crates.getDataFile();
    ...
    public void giveCrateKey(OfflinePlayer offlinePlayer) {
        Set<String> crates = this.crates.getConfig().getConfigurationSection("CRATES").getKeys(false);
    ...
    public void giveCrateKey(OfflinePlayer offlinePlayer, String crateType, Integer amount, boolean showMessage) {
        ...
        if (crate == null) {
            if (offlinePlayer.isOnline()) {
                offlinePlayer.getPlayer().sendMessage(CC.translate(LanguageConfig.CRATE_COMMAND_NOT_FOUND.replace("<name>", crateType)));
            }
            return;
        }
        ...
        if (key == null) {
            if (offlinePlayer.isOnline()) {
                offlinePlayer.getPlayer().sendMessage(CC.translate(LanguageConfig.KEY_NOT_FOUND.replace("<name>", crateType)));
            }
            return;
        }
        ...
        if (offlinePlayer.isOnline()) {
            ...
            if (player.getInventory().firstEmpty() == -1) {
                ...
                if (showMessage) player.sendMessage(CC.translate(LanguageConfig.DO_CRATE_CLAIM));
                return;
            }
            ...
            if (showMessage)
                player.sendMessage(CC.translate(LanguageConfig.RECEIVED_KEY.replace("<crate>", crate.getName())));
        } else {
            HashMap<String, Integer> keys = new HashMap<>();
